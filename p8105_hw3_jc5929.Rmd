...
*_day_id_ is a `r class(pull(accel_data,day_id))` variable
*_day_ is a `r class(pull(accel_data,day))` variable
*_week_ is a `r class(pull(accel_data,week))` variable
*_day_type_ is a `r class(pull(accel_data,day_type))` variable
*_activity_min_ is a `r class(pull(accel_data,activity_min))` variable
*_activity_count_ is a `r class(pull(accel_data,activity_count))` variable
...
```
The ny_noaa data set contains `r nrow(ny_noaa)` rows and `r ncol(ny_noaa)` columns.
Some key varibales inlcudes:
...
```
 ...
 `tamx` and `tmin`. tmax has about `r sum(is.na(ny_noaa$tmax))/nrow(ny_noaa)`% of 
 missing values, and missing values foe tmin accounts approximately for 
 `r sum(is.na(ny_noaa$tmin))/nrow(ny_noaa)`%. `snow` and `snwd` has relatively 
 smaller proportion of missing values. If the data set has a large amount of 
 missing values, it will casue bias, and could influence the accuracr for analysis.
...
Averagr max temperature in July appears to be 25-30°C. There is  
